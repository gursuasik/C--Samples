 ...
 public class Form1 : System.Windows.Forms.Form
 {
  ...
  private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
  {
   ...
   if (ilk_sayfa)
   {
    ...
    {
     son_yazýlan_harf = yazýyor_gibi_yap(e, 
   ...
   RectangleF R = new RectangleF(p.Margins.Left, 
    p.Margins.Top, geniþlik, yükseklik);
   //bu alana kaç harf sýðýdýný bul
   e.Graphics.MeasureString( 
    yazdýrýlacak_metin.Substring(son_yazýlan_harf), 
    fnt, new SizeF(geniþlik, yükseklik), 
    fmt, out harf_sayýsý, out satýr_sayýsý);
   string metin;
   //son yazýlan harften itibaren al
   metin = yazdýrýlacak_metin.Substring(son_yazýlan_harf, harf_sayýsý);
   ...
   if (son_yazýlan_harf < yazdýrýlacak_metin.Length)
   {
    if (printDocument1.PrinterSettings.PrintRange ==
     System.Drawing.Printing.PrintRange.SomePages)
    {
     ...
     else
     {
      ...
      ilk_sayfa = true;
      yazýlan_sayfa_sayýsý = 0;
      son_yazýlan_harf = 0; //Bir sonraki sefer baþtan baþlamasý için
     }
    }
    else
     e.HasMorePages = true;
  ...
  public int yazýyor_gibi_yap(System.Drawing.Printing.PrintPageEventArgs e, int son_sayfa , string yazdýrýlacak_metin)
  {
   ...
   for (sayfa_no = 1 ;  sayfa_no <= son_sayfa ; sayfa_no++)
   {
    ...
    RectangleF R = new RectangleF(p.Margins.Left, p.Margins.Top, 
     geniþlik, yükseklik);
    //bu alana kaç harf sýðýdýný bul
    e.Graphics.MeasureString(yazdýrýlacak_metin.Substring(son_yazýlan_harf), 
